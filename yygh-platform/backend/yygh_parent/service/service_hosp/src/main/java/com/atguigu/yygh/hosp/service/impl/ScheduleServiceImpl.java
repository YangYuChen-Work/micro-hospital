...
public class ScheduleServiceImpl implements ScheduleService {
    ...
    public ScheduleOrderVo getScheduleOrderVo(String scheduleId) {
        ...
        //医院能实体类数据
        Hospital hosp = hospitalService.getHosp(schedule.getHoscode());
        if(hosp == null) {
            throw new YyghException();
        }
        ...
        scheduleOrderVo.setHosname(hosp.getHosname());
        ...
        //封装退号时间
        DateTime quitTime =
                new DateTime(schedule.getWorkDate()).plusDays(bookingRule.getQuitDay());
        scheduleOrderVo.setQuitTime(quitTime.toDate());

        //预约开始时间
        DateTime startTime =
                this.getDateTime(new Date(), bookingRule.getQuitTime());
        ...
        //当天停止挂号时间
        DateTime dateTime = this.getDateTime(new Date(), bookingRule.getQuitTime());
    ...
    @Override
    public List<Schedule> getSuspendScheduleList(String hoscode) {
        List<Schedule> list = scheduleRepository.getScheduleByHoscodeAndStatus(hoscode, -1);
